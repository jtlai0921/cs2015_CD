        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            else if (comboBox1.Text == "所有產品資料")
            {
                var result = from category in db.產品類別
                             ...
                             select category;
                //上面程式也可以使用如下LINQ方法語法
                //var result = db.產品類別.OrderByDescending(m => m.類別編號).ToList();
                foreach (var c in result)
                {
            ...
            else if (comboBox1.Text == "單價大於等於20元的產品")
            {
                ...
                var result = from product in db.產品資料
                             orderby product.單價 ascending
                             where product.單價 >= 20
                             select product;
                //上面程式也可以使用如下LINQ方法語法
                //var result = db.產品資料.OrderByDescending(m => m.單價).Where(m => m.單價 >= 20).ToList();
